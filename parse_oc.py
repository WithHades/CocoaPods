...
def traverse_clang_ast(json_data: dict) -> (list, list):
    """
    traverse the nodes and parse the method and string.
    :param json_data: the data of json format that is from the ast data which generated by clang.
    :return: (method_signs, strings)
    """
    method_signs, strings = [], []
...
def clang(lib_name, lib_version, subspecs_name, global_vals, code_file):
    ...
    if "StringLiteral" not in data and "ObjCMethodDecl" not in data:
        ...
        return
    method_signs, strings = parse_clang_ast(data, data_len, global_vals, code_file)
